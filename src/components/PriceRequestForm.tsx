...
export default function PriceRequestForm({ productId, productName }: PriceRequestFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/leads/price-request', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ productId, name, phone, message }),
      });
    ...
    >
      ...
      {submitted ? (
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          />
          {error && (
            <p style={{ color: '#c0392b', fontSize: '0.8rem', margin: 0, fontFamily: 'var(--font-body, Inter, system-ui, sans-serif)' }}>
              {error}
            </p>
          )}
          <button
